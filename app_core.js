...
function renderStok() {
  ...
  let rows=DB.stok.filter(r=>{
    ...
    if (supFil) { const p=DB.produk.find(x=>x.var===r.var); if(!p||(p.suplaier||'')!==supFil) return false; }
    if (indukFil) { const p=DB.produk.find(x=>x.var===r.var); if(!p||p.induk!==indukFil) return false; }
